...
import { Component, createFactory, createElement, DOM as D } from 'react';
...
export class SideBySideComparison extends Component<Props, State> {
  ...
  private gatherImageMetadata(request: Props): Kefir.Stream<Metadata[]> {
    ...
    const metadataTasks = request.selection
      ...
      .map(iri => queryIIIFImageOrRegion(iri, request.imageIdPattern).flatMap(imageInfo => {
    ...
    return {
      ...
      windowObjects: imagesMetadata.map((metadata): Mirador.WindowObject => ({
        ...
        canvasControls: {
          annotations: {
            annotationState: 'on',
            // TODO: enable when this Mirador issue will be fixed:
            // https://github.com/ProjectMirador/mirador/issues/1209
