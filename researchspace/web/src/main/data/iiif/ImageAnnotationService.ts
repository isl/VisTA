...
import { SparqlClient, SparqlUtil, PatternBinder } from 'platform/api/sparql';
...
const IMAGE_REGION_INFO_QUERY = `
  ...
  select ?type ?imageID ?area ?bbox ?viewport ?svg ?imageIRI {
    ...
    OPTIONAL {
      BIND(?__iri__ as ?region)
      ?region a rso:EX_Digital_Image_Region;
            rdf:value ?svg.
      ?region crmdig:L49_is_primary_area_of ?imageIRI .
      BIND("region" AS ?type)
      OPTIONAL { ?region rso:boundingBox ?bbox }
      OPTIONAL { ?region rso:viewport ?viewport }
    }
...
export function queryIIIFImageOrRegion(
  ...
  const query = SparqlUtil.parseQuery<SparqlJs.SelectQuery>(IMAGE_REGION_INFO_QUERY);
  ...
  return SparqlClient.select(parametrizedQuery).flatMap(selectResult => {
    const bindings = selectResult.results.bindings;
    ...
    if (!type || !imageIRI.isIri()) {
      ...
      return Kefir.constantError<any>(`Invalid image ID '${imageID.value}' ` +
