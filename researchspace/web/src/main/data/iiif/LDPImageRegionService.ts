...
export class LdpRegionServiceClass extends LdpService {
  ...
  public search(objectIri: Rdf.Iri): Kefir.Property<OARegionAnnotation[]> {
    return SparqlClient.select(this.selectForRegions(objectIri)).flatMap(result => {
  ...
  public getRegionFromSparql(regionIri: Rdf.Iri): Kefir.Property<OARegionAnnotation> {
    return Kefir.fromNodeCallback(cb => {
      const req = request
        .post('/sparql')
        .send(this.constructForRegion(regionIri))
      ...
      req.end((err, res) => {
        ...
        jsonld.frame(JSON.parse(res.text), annotationFrame, (err2, framed) => {
          if (err2) {
            cb(err2, framed);
            return;
          }
...
select ?region where {
  ...
  private constructForRegion(regionIri: Rdf.Iri): string {
    return `
