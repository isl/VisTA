...
import {
  PlainTextInput, PlainTextInputProps, SelectInput,
  DatePickerInput, DatePickerInputProps, ResourceEditorForm, ResourceEditorFormProps,
  FieldDefinitionProp, AutocompleteInput, FinalizedModel,
...
class ImageUploadWidget extends Component<Props, State> {
  ...
  private submit(fieldValues: FinalizedModel): Kefir.Property<void> {
    if (this.state.files.length === 0) {
      this.messages.plug(Kefir.constant('You have selected no images.<br>'));
      return;
    }

    const title = fieldValues.values.find(value => value.definition.id === 'title');
    ...
    return parametrizeQueryWithFormValues(
      this.props.config.createResourceQuery, fieldValues
    ).flatMap(query => FileUploadService.uploadFile({
      createResourceQuery: SparqlUtil.serializeQuery(query),
  ...
  render() {
    ...
    return D.div({className: 'iiif-upload__holder'},
                 ...
                 Dropzone(
                   ...
                   D.div({className: 'iiif-upload__description'}, description),
                   D.button(
                     {
                       className: 'iiif-upload__dropzone-button',
                     },
                     'Browse'
                   )
                 ),
      ...
      createElement(ResourceEditorForm,
        ...
        D.button({
            className: 'iiif-upload__cancel-button',
            onClick: () => window.location.reload(),
          },
          'Cancel'
        ),
        D.button({className: 'iiif-upload__submit-button', name: 'submit'}, 'Submit')
      ),
                 this.state.alertState ? createElement(Alert, this.state.alertState) : null,
      progress.map(progress => ProgressBar({ active: true, min: 0, max: 100, now: progress })).getOrElse(null)
    );
...
function parametrizeQueryWithFormValues(
  createResourceQuery: string, fieldValues: FinalizedModel
) {
  return SparqlUtil.parseQueryAsync(createResourceQuery).map((initialQuery: SparqlJs.Query) =>
      fieldValues.values.reduce((query, {definition, values}, key) => {
        const bindings = values.map(value => ({[definition.id]: value}) as _.Dictionary<Rdf.Node>);
