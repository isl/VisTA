...
export interface ComponentProps {
   ...
   *
   * If present (either `Just` or `Nothing`) then it means the component had been created
   * from markup; otherwise it has been created as part of another component.
   *
   * This property should be used only as implementation detail to template-related
   * components (like `SemanticIf`).
   */
  readonly markupTemplateScope?: Data.Maybe<TemplateService.TemplateScope>;
}
...
export abstract class PlatformComponent<P extends ComponentProps, S> extends Component<P, S> {
  ...
  get appliedTemplateScope(): TemplateService.TemplateScope {
    const {markupTemplateScope} = this.props;
