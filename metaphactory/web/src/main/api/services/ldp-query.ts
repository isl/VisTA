...
export class QueryServiceClass extends LdpService {
  ...
  private createGraph(query: Query): Rdf.Graph {
    const {label, value, type} = query;

    const triples = [
      Rdf.triple(
        Rdf.iri(''), rdf.type, sp.Query
      ),
      Rdf.triple(
        Rdf.iri(''), rdf.type, this.getType(type)
      ),
      Rdf.triple(
  ...
  private getType(type: string): Rdf.Iri {
    switch (type) {
      case 'ASK':
        return sp.Ask;
      case 'SELECT':
  ...
  private parseGraphToQuery(iri: Rdf.Iri, graph: Rdf.Graph): Query {
    const queryTypes = [sp.Ask, sp.Select, sp.Describe, sp.Construct].map( qt => qt.value);
    ...
    const type = graph.triples.find(// there are several types, filter only those which are relevant
    ...
    return {
      label,
      value,
      type: this.extractTypeFromIri(type).toUpperCase(),
   ...
   */
  private extractTypeFromIri(type: string): string {
    return /[^#]*$/.exec(type)[0];
  }
}
