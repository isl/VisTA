...
export class ImportResourceComponent extends Component<Props, State> {
  ...
  importFromText = (text: string) => {
    ...
    ldpService
      .importFromText(text, this.props.container)
  ...
  public renderContainerMessage(selectedContainer: string, possibleContainers: any[], isPropsContainerPossible: boolean) {
  ...
  public renderModal() {
    ...
    if (serverDone) {
      ...
      const isPropsContainerPossible = possibleContainers.find(containerIRI => containerIRI['@id'] === this.props.container);
      const canProceed = possibleContainers.length > 0 && (
