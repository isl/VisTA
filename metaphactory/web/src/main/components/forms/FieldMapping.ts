...
import { FieldValue, CompositeValue, FieldError, ErrorKind, DataState } from './FieldValues';
...
import { CardinalitySupport, CardinalitySupportProps } from './inputs/CardinalitySupport';
...
export function mapChildToComponent(
  ...
  if (hasBaseDerivedRelationship(SingleValueInput, element.type)) {
    ...
    (props as CardinalitySupportProps).children = element;
...
export function renderFields(
  ...
  return universalChildren(Children.map(children, child => {
    ...
    if (!mapping) {
      ...
      return createElement(inputType, {...props, ref: onMounted});
    } else if (FieldMapping.isStatic(mapping)) {
      const {staticType, props} = mapping;
      if (props.for) {
        props.definition = model.definitions.get(props.for);
