...
import { SemanticForm, Props as SemanticFormProps } from './SemanticForm';
...
export interface ResourceEditorFormProps {
   ...
   *
   * Also `{{UUID}}` helper available to generate random UUID.
   */
...
export interface ResourceEditorFormState {
 ...
 *
 * @example
 *  <!-- disable storing and recovering intermediate inputs from client persistence layer -->
 *  <resource-editor-form
 ...
 *  <resource-editor-form>
 */
export class ResourceEditorForm extends
  Component<ResourceEditorFormProps, ResourceEditorFormState> {

  private initialState: ResourceEditorFormState;
  ...
  private onSubmit = (e: MouseEvent<HTMLElement>) => {
    ...
    if (readyToSubmit(validatedModel, FieldError.isPreventSubmit)) {
      ...
      const initialModel = this.form.finalize(this.initialState.model);
   ...
   * persistence layer. Return false if - and only if - this.props.browserPersistence
   * is explicitly set to 'false'.
   */
  private browserPersistenceEnabled = (): boolean =>  {
    return this.props.browserPersistence !== false;
  }
