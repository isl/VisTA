...
export class CompositeInput extends SingleValueInput<ComponentProps, {}> {
  ...
  finalizeAsModel(owner: EmptyValue | CompositeValue, value: FieldValue): FinalizedModel {
    const finalizedComposite = this.finalizeSubject(owner, value);

    const finalizedSelf: FinalizedValue[] = FieldValue.isComposite(owner) ? [{
      subject: owner.subject,
      definition: this.props.definition,
      values: [finalizedComposite.subject]
    }] : [];

    const finalizedFields = finalizedComposite.fields.map((state, fieldId) => {
      const input = this.inputs[fieldId];
      return input ? input.finalize(finalizedComposite, state.values) : [];
    }).toArray();

    return {
      subject: finalizedComposite.subject,
      values: finalizedSelf.concat(...finalizedFields),
    };
  }

  finalize(owner: EmptyValue | CompositeValue, value: FieldValue): FinalizedValue[] {
    return this.finalizeAsModel(owner, value).values as FinalizedValue[];
  }
