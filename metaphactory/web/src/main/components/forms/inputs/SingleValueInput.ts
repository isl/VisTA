...
import {
  ...
  EmptyValue, DataState, FieldError, ErrorKind, FinalizedValue,
} from '../FieldValues';
...
export abstract class SingleValueInput<P extends SingleValueInputProps, S> extends Component<P, S> {
  ...
  finalize(owner: EmptyValue | CompositeValue, value: FieldValue): FinalizedValue[] {
    if (!FieldValue.isAtomic(value)) { return []; }
    return [{
      subject: FieldValue.isComposite(owner) ? owner.subject : Rdf.iri(''),
...
export function validationMessages(errors: Immutable.List<FieldError>) {
  const className = 'field-validation-errors';
  const errorClassName = `${className}__error`;
  return D.ul({className}, errors.map((err, index) => D.li({
    key: index,
    className: classnames(
      errorClassName, `${errorClassName}--${FieldError.kindToString(err.kind)}`),
  }, err.message)));
}
