...
export class QueryValidatorComponent extends Component<Props, State> {
  ...
  private validateQuery = (query: string): Kefir.Property<Value> => {
    return SparqlUtil.parseQueryAsync(query).flatMap<Value>( (q: SparqlJs.Query) => {
      const queryType = q.queryType;
  ...
  private getQuery = (): Query => {
    ...
    return {
      ...
      type: query.isJust ? (query.get().value as QueryValue).queryType : '',
    };
  }
