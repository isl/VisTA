...
import { Component, Children, ReactElement, cloneElement, createFactory } from 'react';
...
class SparqlDownloadComponent extends Component<Props, {}> {
  private onSave = () => {
    const results = [];
    SparqlClient.sendStreamSparqlQuery(this.props.query, this.props.header).onValue(
      response => {
