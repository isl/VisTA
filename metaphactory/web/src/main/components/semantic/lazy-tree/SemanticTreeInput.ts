...
import {
  Overlay, InputGroup, Button, Tooltip, OverlayTrigger,
...
export interface SemanticTreeInputProps {
  /**
...
export class SemanticTreeInput extends Component<SemanticTreeInputProps, State> {
  ...
  /**
   * To set initial selection we first need to restore the full path from selected items to the root,
   * we use the same mechanism as we use in search. But in addition to that we also need to
   * fetch labels for selected items using LabelsService.
   */
    ...
    this.cancellation.map(
      ...
      labels => {
        const bindings = _.map(
          initialSelection, iri => ({
            item: iri,
            label: Rdf.literal(labels.get(iri)),
  ...
  private renderTextField() {
    const displaySearchText = this.state.searchInputFocused || this.state.searchText;
    const selected = TreeSelection.leafs(this.state.confirmedSelection);

    const classes = (!displaySearchText && selected.size > 0)
      ? [styles.input, styles.selectedItem] : [styles.input];

    const displayedValue = displaySearchText
      ? (this.state.searchText || '')
      : leafsToCommaSeparatedValues(selected);
    ...
    const textFieldProps: ClearableInputProps & ReactProps<ClearableInput> = {
      ...
      clearTitle: displaySearchText ? undefined : 'Clear selection',
      onClear: () => {
        if (displaySearchText) {
          this.closeDropdown({saveSelection: false});
        } else {
          const confirmedSelection = TreeSelection.empty(EmptyForest.keyOf);
          this.setState({confirmedSelection});
          if (this.props.onSelectionChanged) {
            this.props.onSelectionChanged(confirmedSelection);
          }
        }
      },
    };

    return createElement(ClearableInput, textFieldProps);
  }
  ...
  private renderScrollableDropdownContent(mode: ExpandedMode): ReactElement<any>[] {
    ...
    if (mode.type === 'search') {
      ...
      if (matchLimit && matchedCount === matchLimit) {
        ...
        components.push(D.span({className: styles.searchMessage}, `No results found.`));
      }
    }
