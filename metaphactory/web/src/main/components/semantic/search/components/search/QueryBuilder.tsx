...
import {
  TreeSelection, SemanticTreeInput, Node as TreeNode
} from 'platform/components/semantic/lazy-tree';

import * as Model from '../../data/search/Model';
import * as ModelUtils from '../../data/search/ModelUtils';
import { tryGetRelationPatterns } from '../../data/search/SparqlQueryGenerator';
import {
  ResourceSelectorConfig, HierarchySelectorConfig, SemanticQueryBuilderConfig,
  SEMANTIC_SEARCH_VARIABLES, RESOURCE_SEGGESTIONS_VARIABLES, getConfigPatternForCategory,
  Resource as PatternResource, Text as PatternText,
...
import {
  ...
  TermType,
} from '../../data/search/SearchStore';
import {
  InitialQueryContext, InitialQueryContextTypes,
} from '../../web-components/SemanticSearchApi';
...
class QueryBuilder extends Component<SemanticQueryBuilderConfig, State> {
  ...
  private resourceTermSelector = (searchState: RelationTermSelection) => {
    ...
    {...this.prepareAutoCompletionInputConfig(this.props.resourceSelector, searchState)}/>;
  }
  ...
  private hierarchyTermSelector = (searchState: RelationTermSelection) => {
    ...
    {...this.prepareHierarchySelectorInputConfig(this.props.treeSelector, searchState)}/>;
  }
  ...
  private prepareHierarchySelectorInputConfig(
    hierarchySelectorConfig: HierarchySelectorConfig, searchState: RelationTermSelection
  ...
  private prepareAutoCompletionInputConfig(
    resourceSelectorConfig: ResourceSelectorConfig, searchState: RelationTermSelection
  ): AutoCompletionInputProps {
    const {query, defaultQuery, noSuggestionsTemplate, suggestionTupleTemplate} = resourceSelectorConfig;
    return {
  ...
  private setClauseBindings(searchState: RelationTermSelection, query: string): string {
    ...
    const preparedQuery = SparqlClient.setBindings(parsedQuery, {
    ...
    return SparqlUtil.serializeQuery(preparedQuery);
  }
