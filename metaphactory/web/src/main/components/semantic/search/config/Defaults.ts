...
`;
export const RelationViewTemplate = '{{label}}';
  ...
  export function forResource() {
    return `
      SELECT ?value ?label (COUNT(DISTINCT $subject) AS ?count) WHERE {
        ...
        ?value ${SparqlUtil.preferredLabelPattern()} ?label .
      }
...
export const DefaultFacetRelationsValuesQuery = `
  SELECT ?__relation__ (COUNT( DISTINCT ?subject) AS ?count) WHERE {
    FILTER( EXISTS { ?subject ?__relation__ ?propValue .} ) .
  }
...
export const DefaultFacetRelationTupleTemplate = `
 <div class="facet-relation__content" title="{{$relation.label.value}} ({{count.value}})">
   {{$relation.label.value}} ({{count.value}})
   {{#if $range.thumbnail}}
...
export const DefaultFacetCategoriesValuesQuery =
  `
    PREFIX ssp: <http://www.metaphacts.com/ontologies/platform/semantic-search-profile/>
    SELECT ?__range__ (COUNT(DISTINCT ?subject) AS ?count) WHERE {
      ?prop ssp:hasRange ?__range__.
      OPTIONAL { ?subject ?prop ?propValue . }
