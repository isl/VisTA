...
import { SemanticTreeInputProps } from 'platform/components/semantic/lazy-tree';
...
export interface SemanticQueryBuilderConfig {
  /**
   * Configuration for autosuggestion that is used to select values for relations of 'resource' kind.
   */
  resourceSelector: ResourceSelectorConfig

  /**
   * Configuration for tree-based autosuggestion, that is used to select values for relations of 'resource' kind.
   */
  treeSelector?: SemanticTreeInputProps
...
export type HierarchySelectorConfig = SemanticTreeInputProps;
...
export interface SemanticFacetConfig {
   ...
   *
   * @default <mp-highlight highlight="{{highlight}}">{{label.value}}</mp-highlight> ({{count.value}})
   */
...
export interface FacetCategoryConfig {
  /**
   * SPARQL Select query template returning the list of categories to be displayed and optionally any other metadata e.g the counts of returned results for each category.
   */
  query: string
...
export interface FacetRelationConfig {
  /**
   * SPARQL Select query template returning the list of relations to be displayed and optionally any other metadata e.g the counts of returned results for each relation.
   */
  query: string
 ...
 * returns undefined.
 */
export function getConfigPatternForCategory(config: SemanticSearchConfig, category: Rdf.Iri): PatternConfig | undefined {
