...
class SemanticSearchResult extends Component<ResultComponentProps, State> {
  ...
  private buildResultQuery = (
    ...
    if (this.context.baseConfig.limit) {
      baseQuery.limit = this.context.baseConfig.limit;
    }
    ...
    if (_.has(this.context.bindings, RESULT_VARIABLES.CONTEXT_RELATION_VAR)) {
      this.bindRelationPattern(queryWithBindings, this.context.bindings);
    }

    return SparqlUtil.serializeQuery(queryWithBindings);
      }
    );
  }
