...
export class ResourceThumbnail extends Component<Props, State> {
  ...
  render(): ReactElement<any> {
    ...
    if (this.state.imageUri !== undefined) {
      let imageSrc = this.state.imageUri;
      if (imageSrc === null && !this.props.noImageUri) {
        const fallbackComponent =
          this.findComponent(Children.toArray(this.props.children), NoResourceThumbnail);
        return fallbackComponent;
      } else {
        imageSrc = imageSrc || this.props.noImageUri;
