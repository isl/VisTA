...
import { Component, DOM as D, createFactory, ReactElement, Children, CSSProperties } from 'react';
...
export class PrintComponent extends Component<Props, State> {
  ...
  private loadAndParseTemplate(iri: Rdf.Iri): Kefir.Property<ReactElement<any>> {
    return PageService.loadRenderedTemplate(iri).flatMap<ReactElement<any>>(
      page =>
        Kefir.fromPromise(
