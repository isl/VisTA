...
describe('TemplateService', () => {
  ...
  it('resolves registered partials and helpers', () => {
    const scope = TemplateScope.default.clone();
  ...
  it('uses separate caches for different scopes', () => {
    const firstScope = TemplateScope.default.clone();
    const secondScope = TemplateScope.default.clone();
  ...
  describe('data context', () => {
    ...
    it('restores correct data context inside nested {{#each}} blocks', () => {
      const scope = TemplateScope.default.clone();
      scope.registerPartial('foo',
        '[x={{x}} y={{y}} xindex={{xindex}} yindex={{yindex}}]\n');
